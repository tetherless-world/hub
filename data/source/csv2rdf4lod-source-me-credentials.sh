#3> <#> a <http://purl.org/twc/vocab/conversion/CSV2RDF4LOD_environment_variables> ;
#3>     rdfs:seeAlso 
#3>     <http://purl.org/twc/page/csv2rdf4lod/distributed_env_vars>,
#3>     <https://github.com/timrdf/csv2rdf4lod-automation/wiki/Script:-source-me.sh> .

#source /srv/twc-healthdata/config/triple-store/virtuoso/csv2rdf4lod-source-me-for-virtuoso-credentials.sh
#source /srv/twc-healthdata/config/google-maps/csv2rdf4lod-source-me-for-gmaps-api-key.sh
#source /srv/twc-healthdata/config/ckan/csv2rdf4lod-source-me-for-ckan-api-key.sh
#source /srv/twc-healthdata/data/source/csv2rdf4lod-source-me-when-bioportal-annotating.sh
source /etc/prizms/hub/triple-store/virtuoso/csv2rdf4lod-source-me-for-virtuoso-credentials.sh
source /etc/prizms/hub/ckan/datahub.io/csv2rdf4lod-source-me-for-ckan-credentials.sh
